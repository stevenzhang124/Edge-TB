...
def test_img(net_g, datatest, batch_size):
	...
	accuracy = 1.0 * correct / len(data_loader.dataset)
...
def send_client_weights (weights, path, node_list, connect, clients_layers, forward=None, layer=-1):
	...
	for node in node_list:
		...
		if node in connect:
			addr = 'http://' + connect [node] + path
			data = {'client_layers': clients_layers}
			res = requests.post(addr, data=data, files={'weights': open('../dml_file/client_weights.pkl', 'rb')})
		elif forward:
			addr = 'http://' + connect [node] + path
			data = {'client_layers': clients_layers}
			res = requests.post(addr, data=data, files={'weights': open('../dml_file/client_weights.pkl', 'rb')})
